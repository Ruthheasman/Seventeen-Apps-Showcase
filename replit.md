# Eighteen Apps — Ruth Heasman

A static, single-page brochure showcase for Ruth Heasman's eighteen BSV-focused apps. Visitors land on a full-viewport hero with a draggable 3D fan carousel of all eighteen app cards, then scroll through one generous panel per app. Every app links out to the live product in a new tab.
...
- `artifacts/seventeen-apps/src/data/apps.ts` — **source of truth for all app content.** The ordered list of eighteen apps with name, description, URL and image path, plus `appImageUrl()` for resolving image paths against the Vite base URL
- `artifacts/seventeen-apps/public/apps/` — the eighteen app preview images
...
- **Preview images are real, never generated.** Most come from each app's live Open Graph image; the apps that publish no usable `og:image` were captured with a live screenshot instead. Re-running the fetch script only refreshes the Open Graph ones — it reports the rest as MISS and leaves the captures alone.
- **Preview images are optimised WebP, and the fetch script does not produce them.** Everything in `public/apps/` is WebP, capped at 1600px on the long edge, which keeps the whole set near 1.2 MB instead of 10 MB. The fetch script still writes the original `.jpg`/`.png` it downloads, so after re-running it, convert and re-point the paths in `apps.ts`, or the site will request files that no longer exist:
  `magick <file> -resize '1600x1600>' -strip -quality 78 <name>.webp`
- **Carousel state is one number.** Every card's position, depth, rotation, scale, opacity, blur and z-index derives from a single fractional carousel position clamped from 0 to the last index. There are no independent per-card positions.
...
- Full-viewport hero with the headline and a wide, shallow circular fan of all eighteen app cards, draggable by pointer, trackpad, or arrow keys, with momentum and a settle to the nearest card
...
- Preview image paths must be resolved with `appImageUrl()`. A bare root-relative path like `/apps/foo.webp` escapes the artifact's base path and breaks when the site is not served at the root.
